...
if (isset ($_GET['tid'])) {
    $colname_Rec_detail = $_GET['tid'];
}
mysql_select_db($database_andypos_connect, $andypos_connect);
$query_Rec_detail = sprintf("SELECT type_id FROM menulist WHERE type_id = %s", GetSQLValueString($colname_Rec_detail, "int"));
...
if ($totalRows_Rec_detail == 0) {
    ...
    if ((isset ($_GET['tid'])) && ($_GET['tid'] != "")) {
        $deleteSQL = sprintf(
            "DELETE FROM menutype WHERE type_id=%s",
        ...
        $deleteGoTo = "result_typesave.php";
    ...
    header("Location: result_typedelete.php");
}
